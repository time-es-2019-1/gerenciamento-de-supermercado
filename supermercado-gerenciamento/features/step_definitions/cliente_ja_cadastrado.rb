...
When("Preencho os campos de CPF com {string}, nome com {string}, Ano com {string}, mes {string}, dia {string}, Telefone {string}, Limite de credito {string}, Bairro {string}, Rua {string}, CEP {string}, Numero {string} e o CPF do Funcionario cadastrante {string}") do |cpf, nome, ano, mes, dia, telefone, credito, bairro, rua, cep, numero, cpfFuncionario|
...
When("Clico na opcao criar") do
  click_button 'cadastrar'
end

Then("Vejo que o cliente nao foi criado porque o cpf {string} ja existe para cliente") do |cpf|
  visit '/clientes'
  expect(page).to have_current_path('/clientes')
end